...
import { CrawlJobData, CrawlOptions } from './types.js';
...
export class CrawlerService {
  ...
  public async processJob(job: Job<CrawlJobData>): Promise<void> {
    const { url, depth, crawlId, options } = job.data;

    // 1. Check if already visited or limit reached
    if (await CrawlerState.isVisited(crawlId, url)) return;
